...
LENGTH_H = 12  # 总时长 h
RANDOM_DEMAND_ORIGN_CYCLE = 2  # 周期 h
RANDOM_DEMAND_ORIGN_MAX = 2500
RANDOM_DEMAND_ORIGN_MIN = 1500
...
class Input(object):
    ...
    def _create_demand_o(self):
        delta = RANDOM_DEMAND_ORIGN_CYCLE / 4
        k = 0
        while k * DELTA_T <= LENGTH_H:
            value = (k * DELTA_T) % RANDOM_DEMAND_ORIGN_CYCLE
            if value < delta:
                demand = RANDOM_DEMAND_ORIGN_MIN
            elif value < 2 * delta:
                demand = RANDOM_DEMAND_ORIGN_MIN + (RANDOM_DEMAND_ORIGN_MAX - RANDOM_DEMAND_ORIGN_MIN) / delta * (
                            value - delta)
            elif value < 3 * delta:
                demand = RANDOM_DEMAND_ORIGN_MAX
            else:
                demand = RANDOM_DEMAND_ORIGN_MAX - (RANDOM_DEMAND_ORIGN_MAX - RANDOM_DEMAND_ORIGN_MIN) / delta * (
                            value - 3 * delta)
